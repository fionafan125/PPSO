...
class pso():
    ...
    def optimize_group(self, group, gBest_X, gBest_score):
        for i in group:
            # 更新速度和位置
            r1 = np.random.rand(self.dim)
            r2 = np.random.rand(self.dim)
            ...
            if fitness < gBest_score:
                gBest_score = fitness
                gBest_X = self.X[i, :].copy()
        return gBest_X, gBest_score
    ...
    def parallel_optimize(self):
        ...
        for t in range(self.max_iter):
            ...
            with mp.Pool(2) as pool:
                results = pool.starmap(self.optimize_group, [
                    (group1, self.gBest_X_group1, self.gBest_score_group1),
                    (group2, self.gBest_X_group2, self.gBest_score_group2)
                ])

            # 更新全局最佳解
            self.gBest_X_group1, self.gBest_score_group1 = results[0]
            self.gBest_X_group2, self.gBest_score_group2 = results[1]
            ...
            if t % 100 == 0:
                ...
                record_loss[t] = self.gBest_curve[t]
        
        # 返回最佳解和收斂曲線
        best_solution = (self.gBest_X_group1 if self.gBest_score_group1 < self.gBest_score_group2 else self.gBest_X_group2)
        return self.gBest_curve, best_solution
...
fitnessCurve, para = pso(X, dim=machine_amount, pop_size=60, max_iter=max_iter, LB = LB, UB = UB,w=0.5, c1=2, c2=2).parallel_optimize()
